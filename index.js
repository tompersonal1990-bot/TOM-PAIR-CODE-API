...
// Bulletproof anti-crash global layer
...
app.get('/', (req, res) => {
    res.send('👑 TOM PRIME X - MEMORY BUFFER PAIRING LAYER IS ACTIVE ✅');
});

// Global runtime maps to retain socket states safely in RAM
...
console.log('🚀 Memory-Optimized Telegram Engine Pre-Loaded...');
...
bot.onText(/\/pair/, async (msg) => {
    ...
    setTimeout(async () => {
        try {
            ...
            const { initAuthState, fetchLatestBaileysVersion, makeCacheableSignalKeyStore, delay } = baileys;

            let { version } = await fetchLatestBaileysVersion();
            
            // 🔥 Using pure memory-driven authorization state to bypass Render file-system permission blocks
            const { state, saveCreds } = initAuthState();
