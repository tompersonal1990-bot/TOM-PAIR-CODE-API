/**
 * TOM PRIME X BOT - Dynamic Hybrid Architecture
 * Fixes: ERR_REQUIRE_ESM (100% Dynamic Injection)
 */
...
app.get('/', (req, res) => {
    res.send('👑 TOM PRIME X - DYNAMIC SERVER IS LIVE ✅');
});
...
app.get('/pair', async (req, res) => {
    ...
    try {
        // 🔥 এই সেই ডাইনামিক ইম্পোর্ট যা ERR_REQUIRE_ESM এররকে চিরতরে শেষ করবে
...
try {
    ...
    bot.onText(/\/pair (.+)/, async (msg, match) => {
        ...
        const waitingMsg = await bot.sendMessage(chatId, '⏳ *Please wait...* আপনার রিয়াল নোটিফিকেশন ও কোড জেনারেট হচ্ছে...', {
        ...
        try {
            ...
            if (data.status && data.code) {
                await bot.editMessageText(`✅ *Success!*\n📱 *Number:* \`${cleanedPhone}\`\n🔑 *Code:* \`${data.code}\`\n\n🔔 ফোনের অফিশিয়াল নোটিফিকেশনে ক্লিক করে কোডটি বসিয়ে দিন।`, {
                    chat_id: chatId,
            ...
            await bot.editMessageText(`❌ *API Error:* সার্ভার রেসপন্স ট্র্যাকিং টাইমআউট।`, {
                chat_id: chatId,
                message_id: waitingMsg.message_id
            });
        }
